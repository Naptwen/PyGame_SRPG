...
# ---------------IMAGE---------------
    ...
    def __init__(self, table_window, map_name, screen):
        ...
        self.turn_image = fun_pyImgTxt.pyImgTxt("", [self.screen_w/2, self.screen_h/2], 35,
                                                [255, 255, 255], "banner.png", [500, 200], self.screen)
        self.image_loader()
    ...
    def map_load(self, map_name):
        self.tile_map, self.img_map, self.table = fun_pyMap.map_load_for_game(map_name)
    ...
    def Ai_event(self, Ai_from, Ai_to):
        ...
        for cell in self.table.node_table:
            if cell.type is not None and cell.type.tag == Ai_from:
                ...
                order_list = fun_Ai.Ai_order(
                    cell.pos, Ai_to, self.table, "FOX")
                print("-----------------------------")
                ...
                for order in order_list:
                    ...
                    if order[0] == "target":
                        ...
                        if self.table.giveCell(order[1]).type is not None:
                            self.Ai_target = order[1]
    ...
    def player_mouse_event(self):
        if self.cur_cell_pos:
            cell_pos = [(self.mouse_pos[0] - self.table_screen_pos[0])//self.cell_size,
        ...
        if self.path and self.click_cell_pos and\
    ...
    def map_draw(self):
        for j in range(0, self.table.rows):
            for i in range(0, self.table.cols):
                ...
                if tile > 0:
                    self.screen.blit(self.tile_img_list[tile], [
            ...
            if not (cell.pos in eye_sight):
                ...
                if cell.type is not None:
                    ...
                    y = self.table_screen_pos[1] + cell.pos[1] * \
                        self.cell_size + self.cell_size * 3/4
    ...
    def deploy_draw(self):
        ...
        for cell in self.table.node_table:
            if cell.status == "goal":
                ...
                s.fill([218,165,32])
                self.screen.blit(
                    s, (self.table_screen_pos[0] + cell.pos[0] * self.cell_size,
                        self.table_screen_pos[1] + cell.pos[1] * self.cell_size))
                if cell.type is not None and cell.type.tag == "troop":
                    self.phase = "WIN"
    ...
    def mini_draw(self):
        if self.table is not None\
            ...
            for j in range(0, self.table.rows):
                for i in range(0, self.table.cols):
                    ...
                    if paint > 0:
                        ...
                        mini_window[1] + j * mini_cell[1], 
                        mini_cell[0], mini_cell[1]])
                    status = self.table.node_table[i * self.table.rows + j].status
            ...
            x =  self.table_screen_pos[0] * self.table.cols * mini_size/self.table_window[0]
            y =  self.table_screen_pos[1] * self.table.rows * mini_size/self.table_window[1]
            w =  self.table_window[0] * self.table.cols * mini_size/(self.table.cols * self.cell_size)
            h =  self.table_window[1] * self.table.rows * mini_size/(self.table.rows * self.cell_size) 
            pygame.draw.rect(self.screen,[255,255,255], 
    ...
    def game_event(self):
        ...
        while True:
            ...
            for event in pygame.event.get():
                ...
                cell_pos = [(self.mouse_pos[0] - self.table_screen_pos[0])//self.cell_size,
                            (self.mouse_pos[1] - self.table_screen_pos[1])//self.cell_size]
                if self.phase_check() == "main_title":
                    return "main_title"
                if self.event.type == pygame.QUIT:
                    return "quit"
                if self.deploy == True:
                    if self.event.type == pygame.MOUSEBUTTONDOWN:
                        self.deploy_event(cell_pos)
                    for btn in self.btn2_list:
                        temp = btn.check(self.event, self.mouse_pos)
                        if temp is not None:
                            self.phase = temp
