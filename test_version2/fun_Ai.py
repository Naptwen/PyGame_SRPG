...
def Ai_order(AI_pos, AI_target_tag, Table, AI_TYPE):
    ...
    AI_order = []
    a, b, c, d, e, f= [1,10,1,0,0, True]
    if AI_TYPE == "FOX":
        a, b, c, d, e, f = [1,-10,1,0,2, True]
    elif AI_TYPE == "BEAR":
        a, b, c, d, e, f = [2,-10,3,4,0, True]
    elif AI_TYPE == "TUTLE":
        a, b, c, d, e, f = [10,-10,1,1,0, True]
    factors = [a,b,c,d,e,f]
    target_list = {}
    for target in Table.node_table:
        if target.type is not None and target.type.tag == AI_target_tag:
            ...
            + float(len(AI_path)) * c + float(target.type.atk) * d + float(target.type.ran) * e
            target_list[point] = target.pos
    if target_list: 
        best_target_pos = target_list[max(target_list)]
